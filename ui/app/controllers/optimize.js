...
export default class OptimizeController extends Controller {
  ...
  get activeRecommendationSummary() {
    ...
    if (currentRoute.name === 'optimize.summary') {
      return this.model.find(summary => summary.slug === currentRoute.params.slug && summary.jobNamespace === currentRoute.queryParams.namespace);
    } else {
      return undefined;
    }
  }
  ...
  transitionToSummary(summary) {
    this.transitionToRoute('optimize.summary', summary.slug, { queryParams: { jobNamespace: summary.jobNamespace }});
  }
}
