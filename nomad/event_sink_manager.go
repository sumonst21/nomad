...
START:
	...
	for {
		...
		if err != nil {
			...
			m.logger.Info("received error from managed sink subscription, reloading sink", "error", err)
			goto START
		}
...
func (m *ManagedSink) resetSink() {
	...
	// Set our Sink to the new one
	oldSink := m.Sink
	m.Sink = sink
	m.watchCh = ws.WatchCh(m.stopCtx)

	// If the new Sink value is nil or if the sink has changed in a meaningful
	// way cancel the current subscription.
	if m.Sink == nil || m.Sink.Changed(oldSink) {
